 ...
 */
@RestController//返回给前端字符串或者是json格式数据
@RequestMapping("/community")//映射接受前端地址请求
public class CommunityController {
//    开始写方法
    @Autowired
    private CommunityService communityService;//注入接口

    @RequestMapping("/find")//为什么写路径是为了能够找到他
    public Result find(){
        List<Community> all = communityService.findAll();//调用方法，list自动返回的，=号左边
